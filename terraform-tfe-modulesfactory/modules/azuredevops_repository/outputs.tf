...
output "initial_files" {
  ...
  value       = { for k, v in azuredevops_git_repository_file.this : k => v }
}

output "pipelines" {
  description = "Map of registered ADO pipeline build definitions keyed by name (`merge-pipeline`, `tag-pipeline`, `release-pipeline`)."
  value = {
    merge-pipeline   = azuredevops_build_definition.merge_pipeline
    tag-pipeline     = azuredevops_build_definition.tag_pipeline
    release-pipeline = azuredevops_build_definition.release_pipeline
  }
}
