...
variable "initial_files" {
  description = "(Optional) List of files to commit into the repository immediately after creation. Each entry must specify `path` (relative file path, e.g. `main.tf`) and `content` (file content as a string). Defaults to `[]` (no files pushed)."
  type = list(object({
    path    = string
    content = string
  }))
...
variable "pipeline_name_prefix" {
  description = "(Optional) Prefix prepended to each ADO pipeline display name to ensure uniqueness across the project (e.g. the repository name). Results in names like `<prefix> — merge-pipeline`. Defaults to the repository name (`var.name`)."
  type        = string
  nullable    = false
  default     = ""
}
