...
variable "custom_workspace_access" {
  description = <<EOT
  (Optional) Settings for the team's workspace access within a project.
    ...
    move           : (Optional) The permission to move workspaces into and out of the project.
...
variable "organization_access" {
  ...
  (Optional) Settings for the team's organization access.
    access_secret_teams        : (Optional) Allow members access to secret teams.
    manage_agent_pools         : (Optional) Allow members to create, edit, and delete agent pools.
    manage_membership          : (Optional) Allow members to add/remove users from the organization and visible teams.
    manage_modules             : (Optional) Allow members to publish and delete modules in the private registry.
    manage_organization_access : (Optional) Allow members to update the organization access settings of teams.
    manage_policies            : (Optional) Allow members to create, edit, and delete Sentinel policies.
    manage_policy_overrides    : (Optional) Allow members to override soft-mandatory policy checks.
    manage_projects            : (Optional) Allow members to create and administrate all projects.
    manage_providers           : (Optional) Allow members to publish and delete providers in the private registry.
    manage_run_tasks           : (Optional) Allow members to create, edit, and delete run tasks.
    manage_teams               : (Optional) Allow members to create, update, and delete teams.
    manage_vcs_settings        : (Optional) Allow members to manage VCS Providers and SSH keys.
    manage_workspaces          : (Optional) Allow members to create and administrate all workspaces.
    read_projects              : (Optional) Allow members to view all projects. Requires `read_workspaces = true`.
    read_workspaces            : (Optional) Allow members to view all workspaces.
  EOT
  type = object({
...
variable "workspace_permission" {
  description = <<EOT
  (Optional) Custom permission settings for the team on a specific workspace.
    runs              : (Optional) The permission to grant on the workspace's runs. Valid values are `read`, `plan`, or `apply`.
    variables         : (Optional) The permission to grant on the workspace's variables. Valid values are `none`, `read`, or `write`.
    state_versions    : (Optional) The permission to grant on the workspace's state versions. Valid values are `none`, `read`, `read-outputs`, or `write`.
    sentinel_mocks    : (Optional) The permission to grant on the workspace's generated Sentinel mocks. Valid values are `none` or `read`.
    workspace_locking : (Optional) Whether to grant permission to manually lock/unlock the workspace.
    run_tasks         : (Optional) Whether to grant permission to manage workspace run tasks.
  EOT
  type = object({
    runs              = optional(string, "read")
    variables         = optional(string, "none")
