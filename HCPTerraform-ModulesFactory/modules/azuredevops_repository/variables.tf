...
variable "initial_files" {
  description = "(Optional) List of files to commit into the repository immediately after creation. Each entry requires `path` (relative path, e.g. `main.tf`) and `content` (file content as a string). Defaults to `[]` (no files pushed)."
  type = list(object({
    path    = string
    content = string
  }))
